...
sequenceDiagram
    ...
    participant Docx as template.docx (document.xml)
...
Inside Microsoft Word, write variable names wrapped in `${}`:
...
$templateProcessor->setValue('participant_name', $user->username);
$templateProcessor->setValue('ic_number', $user->ic_num);
$templateProcessor->setValue('organization_name', '蒲公英文教工作坊');
```

## 3. Dynamic Table Row Cloning (`cloneRowAndSetValues`)

For dynamic lists like activity log entries:
